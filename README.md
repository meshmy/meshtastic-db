...
Status: early scaffold. See [AGENTS.md](AGENTS.md) for what currently
exists and how the repo is laid out.

## Quickstart (current scope)

```bash
cp .env.sample .env
cp config/regions.yaml.sample config/regions.yaml   # edit allowed_regions
...
Ingestion services, schema, and the demo dashboard land in later phases —
see [AGENTS.md](AGENTS.md) for current status.
