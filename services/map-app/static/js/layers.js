...
// scrub frame (the standard MapLibre animation pattern) — identity/location
// only. The telemetry value itself is rendered by field.js's separate
// canvas-source overlay, added underneath these layers.
export class NodeLayers {
  ...
  _attach() {
    ...
    // brightness. The marker fill is a fixed neutral color (theme-matched,
    // not value-driven) — only its opacity varies, as a freshness cue (dim
    // when the node has no value at the current scrub instant).
  ...
  // loading to re-add them with theme-appropriate colors, then repopulate
  // from whatever data the caller re-renders next.
  reattach(theme) {
    this.theme = theme;
